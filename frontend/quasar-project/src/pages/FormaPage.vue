    ...
    <q-page class="q-pa-lg flex flex-center">
        <q-card style="max-width: 600px; width: 100%" class="q-pa-xl shadow-4">
            ...
            <q-form @submit.prevent="posaljiFormu" ref="kontaktForm">
                <q-card-section class="q-gutter-lg">
                    ...
                    <q-input v-model="form.email" label="Email" type="email" :rules="[
                        ...
                        val => !val || val.length <= 500 || 'Poruka smije imati najviše 500 znakova'
                    ]" lazy-rules outlined clearable class="q-pa-sm" input-class="text-lg" />
                    <q-input v-model="form.poruka" label="Poruka" type="textarea"
                        :rules="[val => !!val || 'Poruka je obavezna']" outlined lazy-rules clearable autogrow
                        class="q-pa-sm" input-class="text-lg" style="min-height: 200px; max-height: 400px" rows="6" />
                </q-card-section>
            ...
            <q-card-section v-if="obavijest" class="q-pa-none">
                <q-banner class="q-mt-sm" :class="{ 'bg-positive text-white': uspjeh, 'bg-negative text-white': !uspjeh }"
                    dense>
                    {{ obavijest }}
                </q-banner>
            </q-card-section>
